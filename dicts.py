...
def print_melon_at_price(price):
    ...
    for melon in melon_dict.items():
        if melon[1] == price:
            melon_list.append(melon[0])
        
    sorted_melon_list = sorted(melon_list)

    if sorted_melon_list != []:
        for melon in sorted_melon_list:
            print(melon)
    else:
        print("None found")
...
def translate_to_pirate_talk(phrase):
    ...
    for word in line:
        if word in english_to_pirate_dict:
            pirate_phrase_list.append(english_to_pirate_dict[word])
        else:
            pirate_phrase_list.append(word)

    separator = ' '
    return separator.join(pirate_phrase_list)
